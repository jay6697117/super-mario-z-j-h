...
export function createLevel2(){ const rows=18, cols=140; const grid=Array.from({length:rows},()=>Array(cols).fill('-'));
  ...
  // 中途旗
  grid[rows-3][Math.floor(cols/2)]='K';
  ...
  // 炮台（朝左/朝右）
  spawns.push({ type:'cannon', x: 42*TILE_SIZE, y:(rows-3)*TILE_SIZE, dir:-1, period:2.2 });
  spawns.push({ type:'cannon', x: 76*TILE_SIZE, y:(rows-3)*TILE_SIZE, dir:1, period:2.6 });
  spawns.push({ type:'cannon', x: 112*TILE_SIZE, y:(rows-3)*TILE_SIZE, dir:-1, period:2.0 });
  const koopaAt=[44,76,112]; for(const x of koopaAt) spawns.push({type:'koopa',x:x*TILE_SIZE,y:(rows-3)*TILE_SIZE});
  // Lakitu 在中后段出现
  spawns.push({ type:'lakitu', x: Math.floor(cols*0.6)*TILE_SIZE, y: (rows-10)*TILE_SIZE });
