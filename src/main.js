...
function step(dt){ const { level, player, entities, input, physics } = game; input.update(); const wasGrounded = player.grounded; const prevVx = player.vx; if(player.grounded && input.jumpPressed) sfx.jump(); player.update(dt,input,physics,level); if(!input.jump && player.vy<-120) player.vy*=0.7; if(!wasGrounded && player.grounded){ game.particles.spark(player.x+player.w/2, player.y+player.h, '#cbd5e1', 6); } if(Math.abs(prevVx)>150 && Math.sign(prevVx)!==Math.sign(player.vx) && player.grounded){ game.particles.spark(player.x+player.w/2, player.y+player.h, '#94a3b8', 8); } game.time-=dt; if(game.time<=0){ game.time=game.timeMax; player.respawnAtSaved(); showBanner('时间到！继续'); setTimeout(hideBanner,800); } if(game.time<=GAME_CONFIG.lowTimeThreshold) sfx.musicSpeedUp(); const sec=Math.max(0,Math.ceil(game.time)); if(game.lastTimeSec!==sec){ if(sec<=GAME_CONFIG.alertLastSeconds) sfx.alertBeep(); game.lastTimeSec=sec; } updateHUD();
  // 顶砖
  const hh=player.lastHeadHit; if(hh){ const {x,y,tile}=hh; if(tile==='B'){ if(player.powered){ level.set(x,y,'-'); game.particles.burstRect(x*TILE_SIZE,y*TILE_SIZE,TILE_SIZE,TILE_SIZE,'#a16207'); sfx.break(); game.score+=50; updateHUD(); } else sfx.bump(); } else if(tile==='Q'){ level.set(x,y,'N'); const r=Math.random(); if(r<0.6){ const rc=new RewardCoin(x*TILE_SIZE+6,y*TILE_SIZE-8); entities.push(rc); game.coins+=1; game.score+=100; updateHUD(); sfx.coin(); } else if(r<0.8){ entities.push(new Star(x*TILE_SIZE+6,y*TILE_SIZE-8)); sfx.star(); } else { entities.push(new Mushroom(x*TILE_SIZE,(y-1)*TILE_SIZE)); sfx.powerup(); } } }
  // 更新实体
  for(const ent of entities) ent.update?.(dt,physics,level); game.particles.update(dt);
  // 子弹命中
  for(const ent of entities){ if(ent.kind!=='bullet'||ent.dead) continue; for(const target of entities){ if((target.kind!=='enemy'&&target.kind!=='koopa'&&target.kind!=='shell')||target.dead) continue; if(physics.aabbOverlap(ent,target)){ if(target.kind==='koopa'){ const sh=new Shell(target.x,target.y+target.h-22); sh.vx=Math.sign(ent.vx)*240; target.dead=true; entities.push(sh);} else if(target.kind==='shell'){ target.vx=Math.sign(ent.vx)*240; } else { target.dead=true; } ent.dead=true; game.score+=150; updateHUD(); game.particles.spark(target.x+target.w/2,target.y+target.h/2,'#fb923c'); } } }
  // 壳弹射
  for(const sh of entities){ if(sh.kind!=='shell'||Math.abs(sh.vx)<1||sh.dead) continue; for(const target of entities){ if((target.kind!=='enemy'&&target.kind!=='koopa')||target.dead||target===sh) continue; if(physics.aabbOverlap(sh,target)){ target.dead=true; game.score+=200; updateHUD(); game.particles.spark(target.x+target.w/2,target.y+target.h/2,'#22c55e'); } } }
  // 清理
  for(let i=entities.length-1;i>=0;i--) if(entities[i].dead) entities.splice(i,1);
  // 火条碰撞（圆-近似圆）
  for(const ent of entities){ if(ent.kind!=='firebar') continue; const balls = ent.balls(TILE_SIZE); const px = player.x + player.w/2, py = player.y + player.h/2; const pr = Math.min(player.w, player.h)*0.45; for(const b of balls){ const dx=px-b.cx, dy=py-b.cy; if(dx*dx+dy*dy <= (pr+b.radius)*(pr+b.radius)) { if(player.invincibleTime>0){ /* 免疫 */ } else if(player.powered){ player.setPowered(false); } else { player.respawnAtSaved(); return; } } }
  }
  // 玩家与实体
  for(const ent of entities){ if(!physics.aabbOverlap(player,ent)) continue; if(ent.kind==='coin'){ ent.dead=true; game.coins+=1; game.score+=100; updateHUD(); sfx.coin(); } else if(ent.kind==='mushroom'){ ent.dead=true; player.setPowered(true); game.score+=300; updateHUD(); sfx.powerup(); } else if(ent.kind==='star'){ ent.dead=true; player.invincibleTime=10; game.score+=500; updateHUD(); sfx.star(); } else if(ent.kind==='enemy' || ent.kind==='koopa'){ const stomping=player.vy>0 && player.bottom()-ent.top()<16; if(stomping){ if(ent.kind==='koopa'){ ent.dead=true; entities.push(new Shell(ent.x, ent.y+ent.h-22)); } else ent.dead=true; player.vy=-player.jumpSpeed*0.6; game.score+=200; updateHUD(); sfx.stomp(); } else { if(player.invincibleTime>0){ ent.dead=true; game.score+=200; updateHUD(); } else if(player.powered){ player.setPowered(false); player.vy=-player.jumpSpeed*0.5; } else { player.respawnAtSaved(); return; } } } else if(ent.kind==='shell'){ if(Math.abs(ent.vx)<1) ent.vx=player.facing>=0?260:-260; else if(player.invincibleTime<=0){ if(player.powered) player.setPowered(false); else { player.respawnAtSaved(); return; } } } }
  // 旗杆与计分演出
  ...
  if(flagTile){
    if(!game.winStage){
      ...
      game.flagBonus = (ratio>0.9?5000: ratio>0.7?2000: ratio>0.5?800: ratio>0.3?400: 100);
    ...
    if(game.winStage==='slide'){
      if(player.y < game._flagBottom) player.y = Math.min(game._flagBottom, player.y + 220*dt); else game.winStage='count';
    } else if(game.winStage==='count'){
      if(game.time>0){ const drain=Math.min(game.time, 60*dt); game.time-=drain; game.score+=Math.floor(drain*5); sfx.alertBeep(); updateHUD(); }
      else { game.winStage='done'; game.score+=game.flagBonus; updateHUD(); sfx.win(); if(game.currentLevelIndex===0){ showBanner('1-1 通关！R 进入 1-2'); game.currentLevelIndex=1; } else if(game.currentLevelIndex===1){ showBanner('1-2 通关！R 进入 1-3'); game.currentLevelIndex=2; } else { showBanner('全部通关！R 重玩'); game.currentLevelIndex=0; } }
  }
  // 射击
  if(input.shootPressed && player.powered){ const now=performance.now(); if(now-game.lastShotAt>200){ game.lastShotAt=now; const dir=player.facing>=0?1:-1; const b=new Bullet(player.x+player.w/2, player.y+player.h*0.45, dir); entities.push(b); sfx.shoot(); } }
  // 管道
  const pipe=physics.rectFindTile(player, level, (t)=>t==='V'); if(pipe && input.downPressed && typeof level.activate==='function' && level.activeRoom==='main' && level.sub){ level.activate('sub'); game.renderer.setWorldSize(level.cols*TILE_SIZE, level.rows*TILE_SIZE); game.player.x=level.spawn.x; game.player.y=level.spawn.y; game.player.vx=0; game.player.vy=0; if(!level._spawnedSub && level.getSpawns){ for(const e of level.getSpawns()){ if(e.type==='enemy') entities.push(new Enemy(e.x,e.y)); if(e.type==='coin') entities.push(new Coin(e.x,e.y)); if(e.type==='mushroom') entities.push(new Mushroom(e.x,e.y)); if(e.type==='koopa') entities.push(new Koopa(e.x,e.y)); if(e.type==='star') entities.push(new Star(e.x,e.y)); } level._spawnedSub=true; } showBanner('进入地下'); setTimeout(hideBanner,600); }
