...
function step(dt){ const { level, player, entities, input, physics } = game; input.update(); const wasGrounded = player.grounded; const prevVx = player.vx; if(player.grounded && input.jumpPressed) sfx.jump(); player.update(dt,input,physics,level); if(!input.jump && player.vy<-120) player.vy*=0.7; if(!wasGrounded && player.grounded){ game.particles.spark(player.x+player.w/2, player.y+player.h, '#cbd5e1', 6); } if(Math.abs(prevVx)>150 && Math.sign(prevVx)!==Math.sign(player.vx) && player.grounded){ game.particles.spark(player.x+player.w/2, player.y+player.h, '#94a3b8', 8); } game.time-=dt; if(game.time<=0){ game.time=game.timeMax; player.respawnAtSaved(); showBanner('时间到！继续'); setTimeout(hideBanner,800); } if(game.time<=GAME_CONFIG.lowTimeThreshold) sfx.musicSpeedUp(); const sec=Math.max(0,Math.ceil(game.time)); if(game.lastTimeSec!==sec){ if(sec<=GAME_CONFIG.alertLastSeconds) sfx.alertBeep(); game.lastTimeSec=sec; } updateHUD();
  // 顶砖
  const hh=player.lastHeadHit; if(hh){ const {x,y,tile}=hh; if(tile==='B'){ if(player.powered){ level.set(x,y,'-'); game.particles.burstRect(x*TILE_SIZE,y*TILE_SIZE,TILE_SIZE,TILE_SIZE,'#a16207'); sfx.break(); game.score+=50; updateHUD(); } else sfx.bump(); } else if(tile==='Q'){ level.set(x,y,'N'); if(player.powered && !player.canShoot){ entities.push(new Flower(x*TILE_SIZE,(y-1)*TILE_SIZE)); sfx.powerup(); } else { const r=Math.random(); if(r<0.5){ const rc=new RewardCoin(x*TILE_SIZE+6,y*TILE_SIZE-8); entities.push(rc); game.coins+=1; game.score+=100; updateHUD(); sfx.coin(); } else if(r<0.75){ entities.push(new Star(x*TILE_SIZE+6,y*TILE_SIZE-8)); sfx.star(); } else { entities.push(new Mushroom(x*TILE_SIZE,(y-1)*TILE_SIZE)); sfx.powerup(); } } } }
  // 更新实体
  for(const ent of entities) ent.update?.(dt,physics,level,player); game.particles.update(dt);
  // 子弹命中
  ...
  // 壳弹射
  for(const sh of entities){ if(sh.kind!=='shell'||sh.dead) continue; if(Math.abs(sh.vx)>120 && Math.random()<dt*5){ game.particles.spark(sh.x+sh.w/2, sh.y+sh.h, '#9ca3af', 3); }
    if(Math.abs(sh.vx)<1) continue; for(const target of entities){ if((target.kind!=='enemy'&&target.kind!=='koopa')||target.dead||target===sh) continue; if(physics.aabbOverlap(sh,target)){ target.dead=true; game.score+=200; updateHUD(); game.particles.spark(target.x+target.w/2,target.y+target.h/2,'#22c55e'); } } }
  // 清理
  ...
  for(const ent of entities){
    ...
    if(ent.kind==='coin'){
      ent.dead=true; game.coins+=1; game.score+=100; updateHUD(); sfx.coin();
    } else if(ent.kind==='mushroom'){
      ent.dead=true; player.setPowered(true); game.score+=300; updateHUD(); sfx.powerup();
    } else if(ent.kind==='flower'){
      ent.dead=true; player.setPowered(true); player.canShoot = true; game.score+=500; updateHUD(); sfx.powerup();
    } else if(ent.kind==='star'){
      ent.dead=true; player.invincibleTime=10; game.score+=500; updateHUD(); sfx.star();
      ...
      if(stomping){
        if(ent.kind==='koopa'){ ent.dead=true; entities.push(new Shell(ent.x, ent.y+ent.h-22)); } else ent.dead=true;
        player.vy=-player.jumpSpeed*0.6; game.score+=200; updateHUD(); sfx.stomp();
      } else {
        if(player.invincibleTime>0){ ent.dead=true; game.score+=200; updateHUD(); }
        else if(player.canShoot){ player.canShoot = false; player.vy=-player.jumpSpeed*0.3; }
        else if(player.powered){ player.setPowered(false); player.vy=-player.jumpSpeed*0.5; }
        else { player.respawnAtSaved(); return; }
      }
    } else if(ent.kind==='shell'){
      if(Math.abs(ent.vx)<1) ent.vx=player.facing>=0?260:-260;
      else if(player.invincibleTime<=0){ if(player.canShoot){ player.canShoot=false; } else if(player.powered) player.setPowered(false); else { player.respawnAtSaved(); return; } }
    }
  }
  // 旗杆与计分演出
