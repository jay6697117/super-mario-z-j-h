...
export class LevelEditor {
  ...
  renderParams(host){ if(!host) return; const s=this._selected; if(!s){ host.innerHTML=''; return; }
    let html=''; if (s.type==='cannon'){
      html = `方向:<select id="p-dir"><option value="-1">左</option><option value="1">右</option></select> 周期:<input id="p-period" type="number" step="0.1" style="width:60px" /> <button id="p-apply">应用</button> <button id="p-del">删除</button>`;
    ...
    if (s.type==='cannon'){ const dir=byId('#p-dir'); const period=byId('#p-period'); dir.value=String(s.dir ?? -1); period.value=String(s.period ?? 2.2); byId('#p-apply').onclick=()=>{ s.dir=parseInt(dir.value)||-1; s.period=parseFloat(period.value)||2.2; } }
