...
import { TILE_SIZE } from '../constants.js';
  ...
  renderLevelSettings(host){ if(!host) return; const level=this.game.level; if(!level){ host.innerHTML=''; return; }
    const theme=level.theme||'sky'; const tl=level.timeLimit||300; const ltt=level.lowTimeThreshold??''; const als=level.alertLastSeconds??''; const tps=level.timeBonusPerSecond??'';
    host.innerHTML = `主题:<select id="lv-theme"><option value="sky">sky</option><option value="underground">underground</option><option value="castle">castle</option><option value="water">water</option></select> 时间限制:<input id="lv-tl" type="number" step="1" style="width:70px" /> 低时限:<input id="lv-ltt" type="number" step="1" style="width:70px" /> 警报秒:<input id="lv-als" type="number" step="1" style="width:70px" /> 换分倍率:<input id="lv-tps" type="number" step="1" style="width:70px" /> <button id="lv-apply">应用关卡设置</button>`;
    const byId=(id)=>host.querySelector(id);
    byId('#lv-theme').value=theme; byId('#lv-tl').value=String(tl); if(ltt!=='') byId('#lv-ltt').value=String(ltt); if(als!=='') byId('#lv-als').value=String(als); if(tps!=='') byId('#lv-tps').value=String(tps);
    byId('#lv-apply').onclick=()=>{ level.theme=byId('#lv-theme').value; const ntl=parseInt(byId('#lv-tl').value)||tl; level.timeLimit=ntl; const nltt=parseInt(byId('#lv-ltt').value); level.lowTimeThreshold=Number.isFinite(nltt)?nltt:undefined; const nals=parseInt(byId('#lv-als').value); level.alertLastSeconds=Number.isFinite(nals)?nals:undefined; const ntps=parseInt(byId('#lv-tps').value); level.timeBonusPerSecond=Number.isFinite(ntps)?ntps:undefined; this.renderer.setWorldSize(level.cols*TILE_SIZE, level.rows*TILE_SIZE); };
  }
